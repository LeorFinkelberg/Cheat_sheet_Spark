...
Таким образом, больший набор данных не нужно перетасовывать! Однако, таблицу размером больше 8 Гб транслировать не возможно.

\subsection{Соединение большого набора Dataset и набора Dataset средних размеров}

Если меньший из двух наборов данных не помещается в памяти рабочего узла, но его набор ключей помещается, то можно воспользоваться следующим приемом. Так как соединение отбрасывает все элементы большого Dataset-набора, которые не находят пары в Dataset-наборе средних размеров, то мы можем использовать множество ключей Dataset-набора средних размеров перед перетасовкой. Если записей, которые при соединении не найдут пары, много, то такой прием даст ощутимый выигрыш
\begin{lstlisting}[
style = scala,
numbers = none	
]
...
val keys = mediumDf.select("id").as[Int].collect().toSeq // ключи DataFrame-набора средних размеров собираем на драйвере в виде коллекции
print(keys)
/*
...
К примеру, если кластер состоит из 10 узлов со 100 партициями (по 10 партиций на каждый узел), то этот большой массив будет распределен как минимум 100 раз (по 10 раз на каждый узел). Если же использовать широковещательную переменную (\pic{fig:broadcast}), то массив будет передан на \emph{каждый \underline{узел}} \emph{один раз} (с использованием эффективного протокола p2p).
